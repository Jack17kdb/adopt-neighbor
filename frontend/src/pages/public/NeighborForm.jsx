...
export default function NeighborForm() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #2d1a3a 0%, var(--forest) 100%)', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-80px', left: '-80px', width: '400px', height: '400px', borderRadius: '50%', background: 'rgba(200,168,75,0.06)', pointerEvents: 'none' }} />
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 24px 40px' }}>
        <div ref={cardRef} style={{ background: 'white', borderRadius: '24px', padding: '40px', maxWidth: '560px', width: '100%', boxShadow: '0 24px 80px rgba(0,0,0,0.2)' }}>
          <div style={{ marginBottom: '32px', display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
            ...
            <div>
              <h1 className="font-display" style={{ fontSize: '28px', fontWeight: 700, color: 'var(--forest)', lineHeight: 1.1 }}>I Need Help</h1>
              <p style={{ color: 'var(--text-light)', fontSize: '14px', marginTop: '6px', lineHeight: 1.6 }}>You're not alone. Share your details and our caring team will connect you with a local volunteer.</p>
            </div>
          </div>
          <form onSubmit={handleSubmit}>
            <FormField label="Full Name" required><input className="input-field" value={form.name} onChange={e => set('name', e.target.value)} placeholder="John Smith" required /></FormField>
            <FormField label="Email Address" required><input className="input-field" type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="john@example.com" required /></FormField>
            ...
            <FormField label="Home Address" required><input className="input-field" value={form.address} onChange={e => set('address', e.target.value)} placeholder="456 Oak Avenue, Nairobi" required /></FormField>
            <FormField label="What kind of help do you need?" required hint="e.g., grocery shopping, medical appointments, companionship"><textarea className="input-field" value={form.needs} onChange={e => set('needs', e.target.value)} placeholder="I need help with weekly grocery shopping..." required /></FormField>
            <FormField label="Additional comments"><textarea className="input-field" value={form.comment} onChange={e => set('comment', e.target.value)} placeholder="Anything else our staff should know..." style={{ minHeight: '80px' }} /></FormField>
            <button type="submit" className="btn-gold" disabled={loading} style={{ width: '100%', justifyContent: 'center', padding: '14px', fontSize: '15px', marginTop: '8px' }}>
              <Users size={17} />{loading ? 'Submitting...' : 'Submit Request'}
            </button>
          </form>
